...
| 搜索与采集 | ✅ [CollectorAgent.md](/E:/java_study/Mul-agnet/docs/problem/CollectorAgent.md) | ✅ [2026-06-12-search-and-collection-execution-engine.md](/E:/java_study/Mul-agnet/docs/superpowers/plans/2026-06-12-search-and-collection-execution-engine.md) 已按诊断继承、阻塞分级、优化波次重写，旧 Task 轴方案不再作为正式依据；第四轮联合实施进一步把 discovery 路由收口、候选元数据标准化、`Wave 7` 最小采集执行接缝和首个 GitHub API 结构化采集执行器一起落地，第五轮则继续把 `Wave 8` 承接为 `JinaReader` 主路径 + `Playwright FULL_RENDER` 兜底的正式网页采集契约收口，避免把 anti-bot patch 误当作完整答案 | 🟡 首轮三个 blocking 收口包、第二轮自动化契约、第四轮 discovery/collection 联合实施与第五轮网页采集加固均已落地；`Source Family Catalog`、`CollectionExecutionCoordinator`、`CollectorAgent` 新采集骨架、`GithubApiCollectionExecutor`、双路径网页采集执行器、分层提取结果与结构块/质量信号字段已完成自动化收口，但 `Wave 6` 真实 discovery provider 进实链、`news` 家族结构化采集和跨重启 replay 底座仍待推进 | 🟡 2026-06-12 搜索段已通过 dev live 验收（补源/采集/回放/rerun/resume）；2026-06-15 任务 `39` 已补跑第二轮 preview/create/execute/replay/rerun/resume smoke，搜索事实源、回放、重跑与恢复字段通过；2026-06-16 第五轮聚合命令与 `mvn -pl backend test` 已通过，证明 `Wave 8` 双路径网页采集在自动化层已收口，但完整业务质量闭环仍需继续结合真实任务证据质量与最终质检结果验收 |
