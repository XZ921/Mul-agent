...
| 搜索与采集 | ✅ [CollectorAgent.md](/E:/java_study/Mul-agnet/docs/problem/CollectorAgent.md) | ✅ [2026-06-12-search-and-collection-execution-engine.md](/E:/java_study/Mul-agnet/docs/superpowers/plans/2026-06-12-search-and-collection-execution-engine.md) 已按诊断继承、阻塞分级、优化波次重写，旧 Task 轴方案不再作为正式依据；第四轮联合实施进一步把统一 discovery 口径、候选元数据标准化、`Wave 6` owner 边界收口、`Wave 7` 最小采集执行接缝和首个 GitHub API 结构化采集执行器一起落地，第五轮则继续把 `Wave 8` 承接为 `JinaReader` 主路径 + `Playwright FULL_RENDER` 兜底的正式网页采集契约收口，第六轮再把 `Wave 9` 收口为正式 `collectionAudit / collectionReplayTimeline / collectionAuditCheckpoint / 包级 rerun-resume` 闭环，避免把 anti-bot patch 或临时 output 回灌误当作完整答案 | 🟡 首轮三个 blocking 收口包、第二轮自动化契约、第四轮 discovery/collection 联合实施、第五轮网页采集加固与第六轮采集审计/回放/恢复闭环均已落地；`Source Family Catalog`、`CollectionExecutionCoordinator`、`CollectorAgent` 新采集骨架、`GithubApiCollectionExecutor`、双路径网页采集执行器、正式 `collectionAudit` 子域、稳定来源锚点复用与 `collectionAuditCheckpoint` 回填均已完成自动化收口。当前待推进的重点已不再是额外 GitHub vertical discovery provider，而是 `news / rss` 采集专项、跨重启 replay 底座与下游业务质量闭环 | 🟡 2026-06-12 搜索段已通过 dev live 验收（补源/采集/回放/rerun/resume）；2026-06-15 任务 `39` 已补跑第二轮 preview/create/execute/replay/rerun/resume smoke，搜索事实源、回放、重跑与恢复字段通过；2026-06-16 第五轮聚合命令与 `mvn -pl backend test` 已通过，证明 `Wave 8` 双路径网页采集在自动化层已收口；2026-06-17 任务 `43` 在 dev live 中进一步验证 `Wave 9`：`collect_sources_01_02` 首轮暴露 `collectionAudit.summary.totalPackages=4 / successCount=1 / failedCount=3 / status=PARTIAL_SUCCESS`，节点级 `rerun` 后 `collectionAuditCheckpoint` 成功回填进 `nodeConfig`，并得到 `reusedCount=1`，其中 `collect_sources_01_02#002` 明确标记 `reusedFromCheckpoint=true` 与 `checkpointSource=collectionAuditCheckpoint`。同一任务继续 `resume` 并修复 extractor 的 `PricingItem.hasFreeTier` 兼容性后，`extract_schema -> analyze_competitors -> write_report -> quality_check -> rewrite_report -> quality_check_final` 已全部实跑到 `SUCCESS`；但两个 reviewer 节点均返回 `passed=false / requiresHumanIntervention=true / score=0`，说明主停点已从采集与 extractor 兼容性故障迁移到真实质量门禁。完整业务质量闭环仍需继续结合 `Wave 10 / Wave 11 / Wave 12` 推进 |
...
- 方案：`✅` 方案文档已重写完成，[2026-06-12-search-and-collection-execution-engine.md](/E:/java_study/Mul-agnet/docs/superpowers/plans/2026-06-12-search-and-collection-execution-engine.md) 已明确废止旧 Task 轴结构，改为按 `诊断继承 -> 阻塞分级 -> 优化波次 -> 首轮实施裁剪 -> 统一 discovery / 采集边界闭环` 推进，并显式继承“私域垂直 API 主力、公网搜索辅助”的业务优化原则。方案同时把官网、新闻、GitHub 等纳入 `Source Family Catalog` 配置架构，并将 `Wave 6` 改写为“统一公网发现口径 + 主辅语义审计 + discovery/collection owner 边界”，不再要求额外补做 GitHub vertical discovery provider。旧稿不再作为正式方案依据。
- 实施：`🟡` 首轮三个 blocking 收口包（执行真相收口、质量止血、连续性事实源）已于 2026-06-12 落地；第二轮自动化契约收口已于 2026-06-15 完成，覆盖 attemptedTargets、discardedCandidates、稳定 replay timeline、collector insight 直出、preview/runtime source family 同构和排序质量硬化。封板条件中的策略入口统一、HEURISTIC 清理、模板治理、SearchKeywordPolicy、searchAudit、Source Family Catalog 配置骨架，以及 `Wave 5` 对象瘦身、`Wave 6` 统一 discovery 边界收口均已到位。当前未完成的重点转为 `news / rss` 采集专项、跨重启 replay 持久化与最终业务质量闭环，实施列仍不得升 `✅`。
...
- 实链验证：`🟡` 搜索段已于 2026-06-12 通过 dev live 验收；2026-06-17 任务 `43` 已进一步把 `Wave 9` 尾证推进到真实质量门禁，但最终业务质量仍未升绿。
  ...
  未通过：第二轮新增 attempted / discarded / replay timeline / insight / homology / ranking 事实已完成自动化复核，并于 2026-06-15 通过任务 `39` 补跑 dev live API smoke；但 `news / rss` 采集专项、跨重启 replay 持久化与最终业务质量门禁仍未完成。当前全链路未升绿的主因已从采集与 extractor 兼容性故障转为真实证据质量 / 报告质量门禁问题，因此搜索与采集专题仍不能整体升 `✅`。
...
| 搜索执行引擎 | 🟡 已识别 | 优化需求已从搜索与采集链路中浮现；`Wave 6-9` 已把统一 discovery 口径、`CollectionExecutionCoordinator`、正式 `collectionAudit` 子域、`collectionReplayTimeline`、`collectionAuditCheckpoint` 与包级复用语义收口进统一执行骨架，但当前语义仍散落在 `CollectorAgent`、`SearchExecutionCoordinator`、`CandidateVerifier`、`CollectionTargetSelector`、`BrowserSearchRuntimeService` 等协作者中；`Source Family Catalog` 已有骨架，但 `news / rss` 采集专项、跨重启 replay 持久化与下游业务质量闭环仍待继续推进 | [CollectorAgent.md](/E:/java_study/Mul-agnet/docs/problem/CollectorAgent.md)、[2026-06-12-search-and-collection-execution-engine.md](/E:/java_study/Mul-agnet/docs/superpowers/plans/2026-06-12-search-and-collection-execution-engine.md) |
