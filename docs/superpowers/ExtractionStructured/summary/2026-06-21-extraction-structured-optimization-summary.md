...
**改动点**：`SchemaExtractorAgent.buildCollectedContent()` + prompt 模板。
...
**状态**：已在 optimization plan Task 2 中规划，未实施。
...
**改动点**：`SchemaExtractorAgent.doExecute()` + `extractAndNormalize()`。
...
| **P0** | 2.1 0 业务字段语义重试 | extractAndNormalize | 已规划未实施，当前核心阻塞 |
| **P0** | 3.1 extractor 读 dimensions | doExecute + extractAndNormalize | 一行 parseConfig，让 Schema 生效 |
...
| **P2** | 7.2 Provider 内部数据源收口 | 输入 Provider + replay/cache/port | 第二阶段再把 Provider 内部从直接 repository 访问迁移到正式端口与统一输入来源 |
