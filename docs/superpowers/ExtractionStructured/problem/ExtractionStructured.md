...
| `evidenceCoverage` 概览枚举仍偏粗 | reviewer/analysis 已识别 `LLM_REFUSED / EVIDENCE_NOT_COVERING / STRUCTURED_BLOCK_DIRECT`，但 `ReportService` 概览仍聚合为 `TRACEABLE / MISSING_EVIDENCE / EMPTY` | 下游提示能力已经细化，但报告侧 overview 仍是粗粒度汇总。 |
| workflow 下游汇总已起步但还未完全泛化 | `DOWNSTREAM_CONSUMPTION_GAP` 已覆盖“终审 `passed=false`”和“初审明确要求人工补证据”两类 reviewer 阻断 | 仍未完全扩展到所有 analyzer/writer/reviewer/delivery 失败形态的统一归口。 |
| 真实链路验收已取得 task `50` 通过样本，但 live 覆盖仍不完整 | 2026-06-22 task `50` 已从 rerun 后成功通过终审 | 仍缺 `LLM_REFUSED / EVIDENCE_NOT_COVERING / STRUCTURED_BLOCK_DIRECT` 等新状态的 live 命中样本。 |
...
| P2 下游消费与质量门禁 | 2026-06-22 进行中 | `evidenceCoverage` 细化已落地；workflow 已开始把 reviewer 下游阻断归口为 `DOWNSTREAM_CONSUMPTION_GAP`；task `50` live 已验证通过 | 仍需补更广义的 workflow 汇总与更多 live 状态样本 |
...
| 实链验证 | `🟡` | 2026-06-22 task `50` 已完成真实 `extract_schema -> quality_check_final` 链路复验并通过终审；但新 coverage 状态与更多下游失败形态仍缺 live 样本。 |
