...
- [ ] **Step 1: 新增“首轮 0 字段但正文可读时应语义重试”用例**
...
- [ ] **Step 2: 新增“正文为空但结构块非空的证据不能被入口过滤”用例**
...
- [ ] **Step 3: 新增“nodeConfig 的 schemaId / dimensions 应注入 Prompt”用例**
...
- [ ] **Step 4: 新增“Prompt 变量必须拆分结构化证据、质量信号和正文兜底”用例**
...
- [ ] **Step 5: 运行 extractor 定向测试，确认新增用例先失败**
...
- [ ] **Step 1: 让 `extractAndNormalize` 接收 `AgentContext`，并注入分层 Prompt 变量**
...
- [ ] **Step 6: 更新 extractor 默认 Prompt 模板**
...
- [ ] **Step 1: 将 Task 2 中保留的 JSON 解析循环拆成单次业务调用方法**
...
- [ ] **Step 2: 在 `extractAndNormalize` 中追加业务语义重试**
...
- [ ] **Step 1: 更新 `isUsableEvidence`，把结构化证据视为可消费输入**
...
- [ ] **Step 2: 增加 `pageMetadata` 结构化证据判断**
...
```powershell
curl.exe http://localhost:8080/api/task/50 > before-task-50.json
curl.exe http://localhost:8080/api/task/50/nodes > before-task-50-nodes.json
curl.exe http://localhost:8080/api/task/50/replay > before-task-50-replay.json
```
...
```powershell
curl.exe -X POST http://localhost:8080/api/task/50/nodes/extract_schema/rerun
curl.exe http://localhost:8080/api/task/50/nodes
curl.exe http://localhost:8080/api/task/50
curl.exe http://localhost:8080/api/report/50
curl.exe http://localhost:8080/api/report/50/evidences
```
