...
当前阶段：Day 6 质量审查与修订重写文档闭环已完成，3.4 P2 前置协作规划具体执行计划已落地，下一步可开始 P2 Task 1 契约红灯测试
...
| 缺失测试 | 5 个关键测试补齐 |
| 演示证据包 | P1 可复现 Spring Boot/H2 smoke 证据完整；真实外部中间件/live dev 演示脚本可后续增强 |
...
2. 任务开始后，系统生成正式 DAG，每个 Agent 只负责自己的专业产物。
3. 所有关键结论必须带来源，没来源就显式标记证据缺口。
4. Reviewer 只判断质量事实，不再直接决定怎么补图。
5. Orchestrator 根据质量诊断生成编排决策。
6. 决策先过策略校验，再被翻译成动态计划变更。
7. 所有动作进入 trace、checkpoint、replay 和导出审计。
8. 所以系统不是“生成一份报告就结束”，而是能自检、能解释、能补救、能回放。
