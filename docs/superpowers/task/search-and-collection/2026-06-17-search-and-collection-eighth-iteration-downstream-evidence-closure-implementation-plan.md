...
- [ ] 2026-06-19 轻量采集收尾：待完成后冻结搜索采集专题
- [ ] Slice A 下游证据最小闭环：待执行
- [ ] Slice B 报告/质检/交付投影增强：待 Slice A 完成后执行

| 阶段 | 核心目标 | 预期耗时 | 依赖前置条件 | 当前状态 |
| --- | --- | --- | --- | --- |
| Phase A0 | 等待并冻结 2026-06-19 轻量采集收尾 | 0.5-1 天 | Direct/Jina/Playwright 路由验收通过 | 待执行 |
| Phase A1 | 锁定下游证据闭环红灯契约 | 0.5 天 | Phase A0 完成，`ExtractionStructured.md` 已存在 | 待执行 |
| Phase A2 | 新增统一证据视图对象与装配器 | 0.5-1 天 | Phase A1 红灯测试存在 | 待执行 |
| Phase A3 | 收口 extractor 正式输入与 TASK 快照边界 | 1 天 | Phase A2 完成 | 待执行 |
| Phase A4 | 让 analyzer 优先消费 extractor 统一证据视图 | 0.5-1 天 | Phase A3 完成 | 待执行 |
| Phase A5 | Slice A 聚合验证与文档停点 | 0.5 天 | Phase A1-A4 完成 | 待执行 |
| Phase B1 | Writer / Reviewer 消费同一证据视图 | 1 天 | Slice A 已完成 | 待执行 |
| Phase B2 | Report / EvidenceQuery / Export 投影增强 | 1 天 | Phase B1 完成 | 待执行 |
| Phase B3 | dev live 验收与父文档回链 | 0.5-1 天 | Phase B1-B2 完成 | 待执行 |
