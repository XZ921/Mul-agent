...
- [x] 第八轮尚未执行这一事实复核：已完成
- [x] family discovery convergence 前置计划：已完成
- [ ] 第八轮 `Wave 12`：待执行（本计划已完成，可按顺序启动）

| 阶段 | 核心目标 | 预期耗时 | 依赖前置条件 | 当前状态 |
| --- | --- | --- | --- | --- |
| Phase M1 | 锁定 family-first discovery 红灯契约 | 0.5 天 | `架构 1` 已冻结，第八轮尚未执行 | 已完成 |
| Phase M2 | 引入 `SourceFamilyDirectDiscoveryPlanner` 与 catalog 边界 | 0.5-1 天 | Phase M1 完成 | 已完成 |
| Phase M3 | 让 preview discovery 与 runtime initial candidates 共用 planner | 1 天 | Phase M2 完成 | 已完成 |
| Phase M4 | 收口 direct candidate 与 public search supplement 的边界 | 0.5-1 天 | Phase M3 完成 | 已完成 |
| Phase M5 | 自动化复核、文档回链与第八轮执行顺序确认 | 0.5 天 | Phase M1-M4 完成 | 已完成 |
