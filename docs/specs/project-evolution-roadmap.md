...
## 2026-06-26 新暴露的上游契约缺口

本轮额外确认了两类不应直接并入 4.x runtime 的上游问题。它们需要先进入 3.5 诊断、契约冻结与兼容性预埋，而不是通过局部补丁或全链路重构处理。

1. **任务模式契约缺口**：当前任务定义仍默认 `subjectProduct + competitorNames`，导致系统无法正式区分“单产品分析”“双产品横向对比”“本方参照对比”。该问题的优化位置应回到 `任务定义与编排`，后续先冻结 `analysisMode + analysisTargets (+ targetRole)` 这类中性契约，再分阶段传递到搜索 query planner、Analyzer、Writer 与 Reviewer。它不是 ConversationCollaboration 问题，也不是搜索引擎替换问题；搜索发现只是第一个显性受害层。

2. **质量评分口径缺口**：当前质量链路同时存在 `score>=80 and no ERROR issues` 的展示口径、Reviewer 维度化评分、`passed / requiresHumanIntervention` 门禁，以及固定竞品报告字段覆盖规则。该问题的优化位置应归入 `质量审查`，并影响 `报告写作` 与 `交付/审计`。后续应先统一 `score`、`qualityGate`、`deliveryStatus`、`evidenceState` 的语义边界，把 `score` 定义为解释性质量指标，把交付是否放行交给显式门禁，避免通过单纯调低阈值掩盖评分模型失真。

新增这两类缺口后，近期仍保持“诊断先于实现”：任务模式先归档到任务定义与编排专题，质量评分先归档到质量审查专题；只有当这些诊断与交付、审计、运行时问题共同收敛时，才进入后续 4.x 架构判断。
...
**4.x 首轮约束：即使后续决定进入 4.x，也先稳定主业务链路，不把对话协同作为前置阻塞项。首轮 4.x 聚焦 `采集 -> 提取 -> 分析 -> 写作 -> Citation -> 质检 -> 修订/重写 -> 交付/审计` 的 runtime contract 与动态协作闭环；ConversationCollaboration 只作为诊断输入和后续消费端，等主链路 runtime 稳定后再接入为“决策展示 + 安全确认网关 + 受控动作入口”。**
