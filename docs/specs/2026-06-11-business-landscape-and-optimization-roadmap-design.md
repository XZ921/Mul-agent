...
- 已收口事实：`🟡` 第八轮第一阶段已经落地 `DownstreamEvidenceView`、`ExtractResult / drafts / sourceUrls / issueFlags / evidenceFragments / sectionEvidenceBundles` 输出、`CompetitorKnowledge TASK` 快照边界和 `0` 业务字段阻断。`ExecutionPlanDefinitionBuilder` 已把 `dimensions / schemaId` 写入 `extract_schema.nodeConfig`，但 extractor 尚未消费。
- 当前 P0：`🟡` 本迭代只聚焦 extractor 主停点，必须同时覆盖 Prompt 分层、字段级提取指引、0 业务字段语义重试、结构块型证据入口、`schemaId / dimensions` 注入、qualitySignals 翻译和输出强约束。P0 的验收不是“最终报告通过”，而是 extractor 能在已有可读证据或结构块证据下稳定抽出非空业务字段；抽不出时能解释是 `NO_USABLE_EVIDENCE`、`MODEL_OUTPUT_INVALID_JSON`、`NO_BUSINESS_FIELDS_EXTRACTED` 还是字段证据缺口。
- 下一步 P1：`⬜` P0 通过后再收运行态边界：定义 `ExtractorInputProvider / ExtractorInputPackage`，把输入预检、TopK、Prompt 总预算和薄内容诊断前移；analyzer 优先消费 `ExtractResult.drafts + lightweightEvidenceViews`，repository `CompetitorKnowledge TASK` 只做恢复 fallback；`DownstreamEvidenceView / TaskNode.outputData / sharedOutput` 不再携带完整正文。
- 后续 P2：`⬜` P1 通过后再把失败分层扩到 workflow 汇总和下游消费：细化 `evidenceCoverage`，明确 `DOWNSTREAM_CONSUMPTION_GAP` 只由 analyzer / writer / reviewer / delivery 汇总判定；以 task `50` 类真实任务验证 extractor、analyzer、writer/reviewer/delivery 的停点是否已经分层。
- 实链验证：`⬜` 尚未完成真实采集 -> 真提取 -> 真分析 -> 真报告的闭环验收。后续 live 验证必须记录 rerun 前现场、节点级输出、报告证据视图和失败层分类，不能只看任务最终 `SUCCESS/FAILED`。
