...
Tavily 不能用同一个搜索策略覆盖所有来源。`Source Family Catalog` 应决定 Tavily 的 query 模式、是否使用 `include_domains`，以及在证据不足时是否进入受控扩展。官方类证据必须先有锚点，开放网络扩展只能在质量门禁内补充广度，不能替代必要的官方来源。

| Family | 需求 | Query 模式 | include_domains | 结果定位 |
| --- | --- | --- | --- | --- |
| `OFFICIAL` | 精准命中官方来源 | `OFFICIAL_DOCS` 优先；不足时 `TRUSTED_WEB_EXPANSION` | 首轮使用可信官方域名；扩展轮不加 | 官网、协议、规则、官方说明、可信转载或解读 |
| `DOCS` | 精准命中文档/API | `OFFICIAL_DOCS` 优先；不足时 `TRUSTED_WEB_EXPANSION` | 首轮使用可信官方域名；扩展轮不加 | 开放平台、API 文档、帮助中心、可信技术文章 |
| `PRICING` | 官方定价与商业化入口 | `OFFICIAL_DOCS` 优先；不足时 `TRUSTED_WEB_EXPANSION` 辅助 | 首轮使用可信官方域名；扩展轮不加 | 定价页、套餐、商业化说明、可信报价解读 |
...
- 官方和文档类首轮结果不足时，允许追加受控开放网络扩展，但扩展结果必须经过 `trustTier / pageType / qualityTier / contentCompleteness / sourceUrls` 门禁。
- 定价、套餐、协议、规则等强事实结论仍以官方来源为主；第三方材料只能作为辅助背景或交叉验证，不能作为唯一依据。
...
| `TRUSTED_WEB_EXPANSION` | 官方锚点不足时，补可信博客、媒体、研报、技术文章和公告转载 | 不加，靠 Gate 收敛 |
...
`TRUSTED_WEB_EXPANSION` 是官方锚点搜索失败或证据不足后的第二阶段 profile，不是默认放开。触发条件应来自采集后的质量指标，例如 `officialDocHitCount`、`usableContentRatio`、`mediumOrAboveEvidenceCount` 或 Reviewer/Citation 的缺口描述。它不使用 `include_domains`，但必须把候选交给 Gate 做来源分级和噪声剔除。
...
STRONG:
  ...
  && 命中分析主题
  && 覆盖目标竞品或竞品对
  && pageType in [ARTICLE, PDF, OFFICIAL_DOC]
  && trustTier in [HIGH, MEDIUM]
  && sourceUrls 不为空

MEDIUM:
  rawContentLength >= 1500
  && 覆盖至少一个目标竞品
  && pageType in [ARTICLE, PDF, OFFICIAL_DOC, VIDEO_PAGE]
  && trustTier in [HIGH, MEDIUM]
  && sourceUrls 不为空
...
REJECT:
  rawContentLength == 0
  或 pageType in [SEARCH_PAGE, VIDEO_LIST]
  或 SEO 聚合页、课程目录页、纯导航页、无明确作者/机构归属页面
```

可信开放网扩展可以补足竞品分析的解释性材料，例如推荐机制观察、创作者运营规则解读、行业媒体分析和研报片段；但对官方定价、协议、平台规则、接口能力等强事实结论，只能作为辅助材料，不能单独支撑最终结论。
...
对 `TRUSTED_WEB_EXPANSION` 结果还需要额外限制：同一 domain 的可用结果数量应设置上限，避免被单一博客或聚合站点淹没；`FORUM_THREAD / VIDEO_PAGE` 默认只能进入辅助素材，除非当前 family 明确是 `REVIEW` 且存在交叉验证来源。
...
| `SOURCE_DIVERSITY_REPAIR` | `APPEND_DYNAMIC_BRANCH` / `SUPPLEMENT_EVIDENCE` | 可能触发多次 Tavily 调用，例如一次 `OPEN_WEB` 加一次 `OFFICIAL_DOCS`，或官方锚点不足后的 `TRUSTED_WEB_EXPANSION` |
...
- `OPEN_WEB / OFFICIAL_DOCS / TRUSTED_WEB_EXPANSION` 三种主查询 mode，以及定向补证据用的 `EVIDENCE_REPAIR`。
- 基础 DomainHintSet 输入。
...
Experiment C:
  启用 Tavily family-aware 锚点策略：
    OFFICIAL / DOCS / PRICING 首轮使用 include_domains
    NEWS / REVIEW / RESEARCH 不使用 include_domains

Experiment D:
  启用 Tavily family-aware + TRUSTED_WEB_EXPANSION + fallback 保留。
  OFFICIAL / DOCS / PRICING 首轮不足时追加受控开放网络扩展。
  Tavily 失败、超时或 Gate 判定 REJECT 时，自动降级到 DirectHtml / Jina / Playwright。
```

D 组最接近真实生产行为，用来验证“官方锚点优先、可信开放网补广度、原链路兜底”是否能同时提升质量和稳定性。
...
| `trustedExpansionUsableCount` | 受控扩展后通过 Gate 的可信开放网证据数量 | 官方锚点不足场景高于 include_domains-only 策略 |
| `officialAnchorCoverageRatio` | 强事实结论中至少有一个官方锚点来源的比例 | OFFICIAL/DOCS/PRICING 场景必须高于 baseline |
...
6. OFFICIAL/DOCS/PRICING 的强事实结论保留官方锚点来源，不能只由第三方材料支撑
7. thinContentRatio 不高于 baseline
8. collectionFailureRatio 不高于 baseline
9. 所有 REJECT 结果有明确 fastLaneRejectReason
10. playwrightInvocationCount 低于 baseline，用于证明快速通道确实减少了全浏览器兜底调用
```
...
| include_domains 限制发散性 | 官方锚点优先，证据不足时追加 `TRUSTED_WEB_EXPANSION`，并由 Gate 收敛质量 |
