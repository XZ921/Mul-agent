...
#### 4.8.1 当前边界：暂不建模跨字段推理

本阶段的 `FieldAnswerSynthesizer` 明确按字段独立合成结论，不建立全局 evidence graph 或跨字段引用关系。也就是说，一条证据可以在审计数据中被多个字段复用同一个 `sourceUrls / supportingEvidenceIds`，但系统不会自动推导“同一个 structured block 同时支撑 API 能力与隐含计费限制”这类跨字段关系。

典型边界如下：

- 如果某个文档段落同时说明“开放 API 能力”和“开发者每天享有 10000 次免费配额”，分类器可能只产出 `DEVELOPER_DOCS_BLOCK`，也可能在关键词足够明确时产出 `PRICING_BLOCK`；字段结论仍分别由 `coreFeatures` 与 `pricing` 的证据路径独立判断。
- `pricing=REQUIRED` 时，不能因为 `coreFeatures` 已引用同一段 API 文档，就自动认定定价字段已覆盖；必须由 `DOCS_BILLING_OR_LIMITS / OFFICIAL_PRICING_PAGE / TERMS_OR_SERVICE_AGREEMENT` 等字段路径产生可审计状态。
- 如果 Writer 或 Reviewer 发现报告中的一个结论依赖跨字段含义，但对应字段没有完成证据路径，应按 `REPAIR_WITH_TAVILY` 或 `DOWNGRADE_FIELD` 处理，而不是让 LLM 自行补全引用关系。

因此，跨字段推理是后续迭代项。后续可新增 `CrossFieldEvidenceLinker` 或 `EvidenceGraphBuilder`，以 `evidenceId -> supportedFields -> inferredRelations -> reviewerWarnings` 的形式建模复合证据；但本轮不把它作为 task66 通过标准，避免把字段契约、repair 和结构化分类的主线复杂化。
...
当前 `TavilyPrefetchedContentBlockClassifier` 的定位是轻量规则分类器，不是完整内容理解系统。它依赖显式关键词、段落长度和防噪门槛，因此会有可接受的漏判边界：如果定价或配额信息使用非标准表达，例如“开发者每天享有 10000 次免费配额”，但正文没有出现“计费、定价、套餐、收费、pricing、billing”等显式信号，分类器可以不生成 `PRICING_BLOCK`。这类漏判应由字段证据路径和 repair 承接，而不是降低关键词门槛去赌语义理解。

安全策略如下：

- 对规则分类器，误判比漏判更危险；误生成 `PRICING_BLOCK` 会让 Reviewer 误以为定价字段已有结构化证据。
- 对非标准定价表达，优先保留 `NO_STRUCTURED_BLOCKS` 或仅输出低可信正文信号，并记录 `blockEvidenceReason`，让 `pricing` 字段路径继续检查 `DOCS_BILLING_OR_LIMITS`。
- 当 `pricing=REQUIRED` 且未发现 `PRICING_BLOCK`，系统应触发 `REPAIR_WITH_TAVILY` 或继续运行字段证据路径，而不是把字段标记为 `TRACEABLE`。
- 后续如要提升召回，应先扩展 `AnalysisDimensionMappingCatalog / queryIntents / evidencePath`，或引入可审计的模型分类器；不能把规则分类器改成过宽泛的关键词匹配。
...
- 非标准定价/配额表达未命中 `PRICING_BLOCK` 时，允许视为规则分类器漏判；验收重点是字段证据路径能继续触发 repair 或输出可审计未覆盖状态，不能为了提高召回放宽防噪门槛。
- 同一条证据被多个字段引用时，当前只要求字段级 `sourceUrls / supportingEvidenceIds` 可复用；不要求自动建立跨字段推理关系，也不得把其他字段的覆盖状态借用为本字段已覆盖。
...
- 暂不建立跨字段 evidence graph；一条证据可被多个字段复用，但字段结论仍独立合成和审查。
- 暂不要求规则分类器理解所有非标准定价表达；漏判通过字段证据路径和 repair 闭环处理，误判必须继续被防噪门槛压住。
...
16. 新增 `FieldAnswerSynthesizer`，把多条字段证据路径合成为可审计 `FieldAnswerConclusion`，记录 reasoning steps、sourceUrls、contradictions 和 recommendedNextAction；本阶段只做字段内合成，不做跨字段 evidence graph。
