...
> **🔻 降级标注（2026-07-02）：本文的「§3 改动方案（档 A/B）」已由 `2026-07-02-12-search-first-routing-inversion-and-quota-calibration-plan.md` 承接并细化执行，不再作为改动方案来源。**
> 本文自此**降级为诊断/定性依据文档**，保留价值在:§1 问题定性(两套设计哲学打架)、§2 病灶代码定位、§5 风险评估——这些是 plan 12 的依据来源。
> 避免两份文档都声称自己是"改动方案"导致改一处忘一处:**改动方案只看 plan 12,本文只作背景参考。**
